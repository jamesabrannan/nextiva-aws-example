...
// Modified for Nextiva Connect Recording Application

// allow spawning child processes and use the S3Uploader from upload.js
...
// file name to save the recording as, including extension.

const RECORD_FILE_NAME = args[3];
logger.log('info','[recording process] RECORD_FILE_NAME: ${RECORD_FILE_NAME}');
...
transcodeStreamToOutput.stderr.on('data', data => {
    logger.log('error', `[transcodeStreamToOutput process] stderr: ${(new Date()).toISOString()} ffmpeg: ${data}`);
});

// the file name to save the recording

const fileName = RECORD_FILE_NAME;
...
process.on('SIGTERM', (code, signal) => {
    logger.log('info', `[recording process] exited with code ${code} and signal ${signal}(SIGTERM)`);
    process.kill(transcodeStreamToOutput.pid, 'SIGTERM');
});

// debug use - event handler for ctrl + c
process.on('SIGINT', (code, signal) => {
    logger.log('info', `[recording process] exited with code ${code} and signal ${signal}(SIGINT)`)
    process.kill('SIGTERM');
});

process.on('exit', function(code) {
    logger.log('info', '[recording process] exit code', code);
});