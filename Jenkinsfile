    ...
    stage('Configure S3 Buckets')
    {
        steps {
            ...
            try {
             ...
             try {
                // block public access
                sh "${DOCKER_AWS_CMD} s3api put-public-access-block --bucket ${S3_BUCKET_LOG} --public-access-block-configuration 'BlockPublicAcls=true,IgnorePublicAcls=true,BlockPublicPolicy=true,RestrictPublicBuckets=true'"
            }
            catch(err){
                echo ${err}
